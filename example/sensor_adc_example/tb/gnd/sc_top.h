 ...
 * 
 * @file    sensor_adc_example/tb/gnd/sc_top.h
...
SCA_TDF_MODULE(sca_tdf_stimuli)
{
	...
	void processing()
	{
		...
		//input port
		std::cout<< " readout gnd-pout[gnd] = " << pin_gnd.read()<<std::endl; 
	}
};
...
SC_MODULE(sc_top) 
{
	...
	{ 
		...
		tf = trace_format.compare("vcd") == 0 ?
			sca_util::sca_create_vcd_trace_file("tracefile_gnd_tb"):
			sca_util::sca_create_tabular_trace_file("tracefile_gnd_tb");
		...
		i_stimuli->pin_gnd(sig_from_gnd_gnd);
